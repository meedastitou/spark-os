...
// constructor
const hplModbusMulticlient = function hplModbusMulticlient(log, machine, model, conf, db, alert) {
  ...
  let holdingRegisterErrorResponseTimer = null;

  const SLAVEID = 0;
  const TYPE = 1;
  const ADDR = 2;
  const HIGH_BYTE_FIRST = 3;
  const HIGH_WORD_FIRST = 4;
  const SWAP_CHARACTER_PAIRS = 5;
  const FORMAT = 6;
  const COUNT = 7;
  ...
  function readTimer() {
    ...
    //    console.log('---readAll: variableReadArray = ' + JSON.stringify(variableReadArray));
    ...
    readAll(variableReadArray, (err, dataArray) => {
      ...
      //      console.log('---response: dataArray = ' + JSON.stringify(dataArray));
  ...
  function readAll(variableArray, done) {
    ...
    if (sendingActive === true) {
      // console.log('!!!!!!! readAll returning early - still being busy from last read');
      return done(new Error('readAll returning early due to still being busy from last read'), {});
    }
    ...
    optimizedRequestArray.push([variableArray[0].slaveId,
      variableArray[0].type,
      parseInt(variableArray[0].address, 16),
      variableArray[0].highByteFirst,
      variableArray[0].highWordFirst,
      variableArray[0].swapCharacterPairs,
      variableArray[0].format,
    ...
    for (let i = 1; i < variableArray.length; i += 1) {
      ...
      let optimized = false;
      if ((optimize === true)
        && (variable.slaveId === variableArray[i - 1].slaveId)
        && (variable.type === variableArray[i - 1].type)
        && (variable.highByteFirst === variableArray[i - 1].highByteFirst)
        && (variable.highWordFirst === variableArray[i - 1].highWordFirst)
        && (variable.swapCharacterPairs === variableArray[i - 1].swapCharacterPairs)
      ...
      if (!optimized) {
        optimizedRequestArray.push([variable.slaveId,
          variable.type,
          parseInt(variable.address, 16),
          variable.highByteFirst,
          variable.highWordFirst,
          variable.swapCharacterPairs,
          variable.format,
          variable.length === undefined ? 1 : variable.length]);
      }
    }
    ...
    let doneFlag = false;
    async.whilst(
      () => (!doneFlag) && (requestIndex < optimizedRequestArray.length),
      (callback2) => {
        ...
        const item = optimizedRequestArray[requestIndex];

        client.setID(item[SLAVEID]);
        ...
        switch (item[TYPE]) {
          ...
          case 'hr': { // holding registers
            // 16bit (or more if bonded) so look at format property
            // to see what length we need to get
            let length;
            if (item[FORMAT] === 'char') {
            ...
            // console.log('------readHoldingRegisters: SLAVEID:ADDR = ' +
            //             item[SLAVEID] + ':' + item[ADDR] + '     length = ' + length);
            client.readHoldingRegisters(item[ADDR], length, (err, modbusData) => {
              // console.log('------response: err = ' + JSON.stringify(err) +
              //             'modbusData = ' + JSON.stringify(modbusData));
              let i = 0;

              if (holdingRegisterErrorResponseTimer) {
                clearTimeout(holdingRegisterErrorResponseTimer);
                holdingRegisterErrorResponseTimer = null;
              }

              if (err) {
                if (interfaceType === 'serial') {
                  if (err.message.lastIndexOf('CRC error') !== -1) {
                    // console.log('------CRC error - briefly wait for correct packet');
                    // we 'should' get a second immediate callback when we see the CRC error.  this
                    // appears to be a bug in the modbus library - it delivers a callback
                    // for the crc, then a second with the actual data.
                    // just in case, let's set a timer here to force the callback
                    // so we don't just wait forever.
                    holdingRegisterErrorResponseTimer = setTimeout(() => {
                      // onsole.log('-----------EXECUTED CRC error timeout-----------');
                      callback2(null);
                    }, 100);
                    return;
                  }
                ...
                if (err.message.lastIndexOf('Timed out') !== -1) {
                  ...
                  // // if we timed out we should not try reading the other vars
                  // return callback2(err);
                  // .log('------Timeout error - briefly wait for correct packet');
                  holdingRegisterErrorResponseTimer = setTimeout(() => {
                    // console.log('-----------EXECUTED Timeout error timeout-----------');
                    callback2(null);
                  }, 100);
                  return;
                } if (err.message.lastIndexOf('Port Not Open') !== -1) {
              ...
              // console.log('modbusData = ' + JSON.stringify(modbusData));
              // console.log('modbusData.buffer = ' + JSON.stringify(modbusData.buffer));
              // console.log('modbusData.buffer.length = ' + modbusData.buffer.length);
              // console.log('modbusData.buffer.data = ' + JSON.stringify(modbusData.buffer.data));
              // console.log('modbusData.buffer.data.length = ' + modbusData.buffer.data.length);
              if (modbusData.buffer.length !== (length * 2)) {
                // console.log('------mismatched response buffer length - retry');
                callback2(null);
                return;
              }
              ...
              if (item[FORMAT] === 'char') {
                ...
                for (i = 0; i < item[COUNT]; i += 1) {
                  ...
                  } else if (item[HIGH_WORD_FIRST] === false) {
                ...
                for (i = 0; i < item[COUNT]; i += 1) {
                  if ((item[HIGH_BYTE_FIRST] === true)
                   && (item[HIGH_WORD_FIRST] === true)) {
                    // if fully big endian
                    resultsArray[counter] = modbusData.buffer.readFloatBE(i * 4);
                  } else if ((item[HIGH_BYTE_FIRST] === false)
                   && (item[HIGH_WORD_FIRST] === false)) {
