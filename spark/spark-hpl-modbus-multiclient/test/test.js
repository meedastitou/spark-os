...
const testMachineLowByteLowWordNoSwap = {
  info: {
    name: 'test-machine',
...
const testMachineLowByteHighWordNoSwap = {
  info: {
    name: 'test-machine',
    fullname: 'Test machine',
    version: '1.0.0',
...
const testMachineHighByteLowWordSwap = {
  info: {
    name: 'test-machine',
    fullname: 'Test machine',
    version: '1.0.0',
...
const testMachineHighByteHighWordSwap = {
  info: {
    name: 'test-machine',
    fullname: 'Test machine',
    version: '1.0.0',
...
describe('Spark HPL Modbus', () => {
  ...
  it('successfully create a new Modbus HPL', (done) => {
    ...
    sparkHplModbus = new SparkHplModbus.hpl(
      log.child({ machine: testMachineLowByteLowWordNoSwap.info.name }),
      testMachineLowByteLowWordNoSwap,
      testMachineLowByteLowWordNoSwap.settings.model,
      sparkConfig,
      null,
      sparkAlert.getAlerter(),
    );
    sparkHplModbus.tester.prototype.setVariables(testMachineLowByteLowWordNoSwap.variables);
  ...
  it('spark HPL modbus should produce data low byte first, low word first', (done) => {
    const readVariables = [];
    const gotDataForVar = [];
    testMachineLowByteLowWordNoSwap.variables.forEach((variable) => {
        ...
        if (_.get(variable, 'access', 'read') === 'read') {
          if (variable.name === data.variable) {
            if (gotDataForVar.indexOf(data.variable) === -1) {
              console.log(`reading variable ${variable.name}`);
  ...
  it('successfully create a new Modbus HPL', (done) => {
    /* eslint new-cap: ["error", { "newIsCap": false }] */
    sparkHplModbus = new SparkHplModbus.hpl(
      log.child({ machine: testMachineLowByteHighWordNoSwap.info.name }),
      testMachineLowByteHighWordNoSwap,
      testMachineLowByteHighWordNoSwap.settings.model,
      sparkConfig,
      null,
      sparkAlert.getAlerter(),
    );
    sparkHplModbus.tester.prototype.setVariables(testMachineLowByteHighWordNoSwap.variables);
    return done();
  });
  ...
  it('spark HPL modbus should produce data low byte first, high word first', (done) => {
    ...
    const readVariables = [];
    const gotDataForVar = [];
    testMachineLowByteHighWordNoSwap.variables.forEach((variable) => {
      if (!_.get(variable, 'machineConnected', false)
       && (_.get(variable, 'access', 'read') === 'read')) {
        readVariables.push(variable);
      }
  ...
  it('successfully create a new Modbus HPL', (done) => {
    /* eslint new-cap: ["error", { "newIsCap": false }] */
    sparkHplModbus = new SparkHplModbus.hpl(
      log.child({ machine: testMachineHighByteLowWordSwap.info.name }),
      testMachineHighByteLowWordSwap,
      testMachineHighByteLowWordSwap.settings.model,
      sparkConfig,
      null,
      sparkAlert.getAlerter(),
    );
    sparkHplModbus.tester.prototype.setVariables(testMachineHighByteLowWordSwap.variables);
    return done();
  });
  ...
  it('spark HPL modbus should produce data high byte first, low word first', (done) => {
    ...
    const readVariables = [];
    const gotDataForVar = [];
    testMachineHighByteLowWordSwap.variables.forEach((variable) => {
      if (!_.get(variable, 'machineConnected', false)
       && (_.get(variable, 'access', 'read') === 'read')) {
        readVariables.push(variable);
      }
  ...
  it('successfully create a new Modbus HPL', (done) => {
    /* eslint new-cap: ["error", { "newIsCap": false }] */
    sparkHplModbus = new SparkHplModbus.hpl(
      log.child({ machine: testMachineHighByteHighWordSwap.info.name }),
      testMachineHighByteHighWordSwap,
      testMachineHighByteHighWordSwap.settings.model,
      sparkConfig,
      null,
      sparkAlert.getAlerter(),
    );
    sparkHplModbus.tester.prototype.setVariables(testMachineHighByteHighWordSwap.variables);
    return done();
  });
  ...
  it('spark HPL modbus should produce data high byte first, high word first', (done) => {
    ...
    const readVariables = [];
    const gotDataForVar = [];
    testMachineHighByteHighWordSwap.variables.forEach((variable) => {
      if (!_.get(variable, 'machineConnected', false)
       && (_.get(variable, 'access', 'read') === 'read')) {
        readVariables.push(variable);
      }
  ...
  it('update model should succeed selecting combined data mode', (done) => {
    sparkHplModbus.tester.prototype.setEndedness(false, false);
    sparkHplModbus.updateModel({
      enable: true,
      requestFrequency: 0.01,
  ...
  it('an alert should be raised and connection variable set after an error caused', (done) => {
    ...
    sparkAlert.on('raise', (alert) => {
      ...
      alert.key.should.equal(`${testMachineHighByteHighWordSwap.info.name}-connectivity-alert`);
      alert.msg.should.equal(`${testMachineHighByteHighWordSwap.info.name}: Unable to open connection`);
  ...
  it('update model should raise an alert if serial mode invalid', (done) => {
    sparkAlert.on('raise', (alert) => {
      ...
      alert.should.have.all.keys('key', 'msg', 'description');
      alert.key.should.equal(`${testMachineHighByteHighWordSwap.info.name}-connectivity-alert`);
      alert.msg.should.equal(`${testMachineHighByteHighWordSwap.info.name}: Unable to open serial port`);
